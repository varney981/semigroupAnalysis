    ...
    for i in range(thing):
        ...
        for a in range(tbl.order):
            ...
            if v != None and w != None:
                ...
                #output = ""
                #output += "b    = {0}, c    = {1}, a   = {2}\n".format(b,c,a)
                #output += "vcab = {0}, cabw = {1}\n\n".format(tbl.multiply([v,c,a,b]),tbl.multiply([c,a,b,w]))
                #print output

                ##END DEBUG
        if not first_set:
            if a_set == new_a_set:
                print 'well it is the same a set: ' + "prev = {0}, new = {1}".format(len(a_set),len(new_a_set))
            else:
                print "prev = {0}, new = {1}".format(len(a_set),len(new_a_set))
        a_set = new_a_set
        first_set = False
