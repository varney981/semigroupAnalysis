...
class mat(object):
    ...
    def __add__(self,other):
        ...
        #print "time in addition generating matrix: " + str(time.time() - start_time2)
        #print "time in addition total: " + str(time.time() - start_time1)
...
P = P[start:]
a_list = []
f_debug = open('counter1.txt', 'w')
results = open('results1.txt', 'w')
f_debug.close()
results.close()
while len(P) > 0:
    now = time.time()
    f_debug = open('counter1.txt', 'a')
    f_debug.write("There are " + str(len(P)) + " pairs to go!\n")
    f_debug.close()
    T = set()
    count = 0
    if incr < len(P):
    ...
    for i in range(thing):
        ...
        for a in range(tbl.order):
            ...
            v = None
            w = None
            if (b in tbl.cTable[:,cab]) and (c in tbl.cTable[cab,:]):
            ...
            if v != None and w != None:
                ...
                new_a_set.add(a)
            #v = None
            #for _v in range(tbl.order):
                #vcab = tbl.multiply([_v,cab])
                #if b == vcab:
                    #v = _v
                    #break
            #if v == None:
...
#
            #w = None
            #for _w in range(tbl.order):
                #cabw = tbl.multiply([cab,_w])
                #if c == cabw:
                    #w = _w
                    #break
            #if w == None:
...
#
        ...
        #if not first_set:
            #if a_set == new_a_set:
                #print 'for (b,c) = ' + str((b,c)) + ": same set     , prev = {0}, new = {1}".format(len(a_set),len(new_a_set))
            #else:
                #print 'for (b,c) = ' + str((b,c)) + ": different set, prev = {0}, new = {1}".format(len(a_set),len(new_a_set))
        #a_set = new_a_set
        #if not(new_a_set in a_list):
            #print 'got a new one! now at ' + str(len(a_list) + 1) 
            #a_list.append(new_a_set)
        #first_set = False


#Step 5: The real test
    #print str(time.time() - now) + " seconds spent searching for T with P of " + str(incr)
    I = mat.generateMatrixFromMatrix(numpy.identity(mat.dimension,dtype=int))
    count = 0
    time_converting = 0
    time_adding = 0 
    #print "doing this many in T: " + str(len(T))
    for (a,b,c,v,w,y) in T:
        ...
        time_converting += time.time() - start_time
        ...
        for (p,q,r,s,t,u) in product(range(2), repeat=6):
            ...
            time_adding += time.time() - start_time
            if (ya == ya_test) and (ay == ay_test):
                ...
                output += "p = {0}, q = {1}, r = {2},\ns = {3}, t = {4}, u = {5}\n\n".format(p,q,r,s,t,u)
                results = open("results1.txt", 'a')
                results.write(output)
                results.close()


    P = P[incr:]
    #print str(time.time() - now) + " seconds to do " + str(incr)
    #print str(time_converting) + " was spent in conversion"
    #print str(time_adding) + " was spent adding"
