...
for (a , b, r, s) in product(range(order), repeat=4):
    rs = groupTable.multiply([r,s])
    sr = groupTable.multiply([s,r])
    if(not(a == rs and b == sr)):
        continue

    pq_found = False
    for (p, q) in product(range(order), repeat=2):
        pq = groupTable.multiply([p,q])
        qp = groupTable.multiply([q,p])
        qbp = groupTable.multiply([q,b,p])
        if(pq == unitElement and unitElement == qp and a == qbp):
            pq_found = True
            break

    if(not pq_found):
        print_results(a, b, r, s, filename)
