...
# function: populate a list with all rank values for matrices
def generate_rank_values(order):
    result = list()
    for i in range(order):
        M = mat.generateMatrixFromNumber(i)
        result.append(numpy.linalg.matrix_rank(M.matrix))
    return result
...
#generate ranks for all matrices
all_ranks = generate_rank_values(order)

#begin testing selected matrices
fh = open('Q11Results.txt', 'w');
unitElement = groupTable.unitElement()
for (a , b) in product(range(order), repeat=2):

    if(a == 0 or b == 0 or all_ranks[a] != all_ranks[b]):
        continue
