...
for tbl in tbls:
    ...
    for (e,f) in product(range(order), repeat=2):
        e_2 = tbl.multiply([e,e])
        f_2 = tbl.multiply([f,f])
        if not (e_2 == e and f_2 == f):
            continue

        for (r,s) in product(range(order), repeat=2):
            rs  = tbl.multiply([r,s])
            sr  = tbl.multiply([s,r])
            if not(e == rs and f == sr):
                continue
            ...
            for (p,q) in product(range(order), repeat=2):
                pq = tbl.multiply([p,q])
                qp = tbl.multiply([q,p])
                qfp = tbl.multiply([q,f,p])
                if(pq == unitElement and qp == unitElement and e == qfp):
                    pq_found = True
                    break
