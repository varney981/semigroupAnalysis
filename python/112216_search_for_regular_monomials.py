...
###START OF FUNCTIONS
def reduceTerm(term, reduce_terms):
    old_string = m+u+m
    new_string = ''
    start = True
...
u_degree_string = raw_input("What are the desired deg(u) (enter as 'i,j,k,...'? >> ")
special_str = "degree" + degree + "_u_"
degree = int(degree)
u_degree = []
for u in u_degree_string.split(','):
    u_degree.append(int(u))
    special_str += u + '_'
...
# perform test G to determine valid u for testing
test_u = {}
removal_pairs = [ay,ya,bh,hb,cg,gc]
for monomial_char_tuple in product('abcghy', repeat=degree):
    monomial_char_list = list(monomial_char_tuple)
    m = ''.join(monomial_char_tuple)
    m = reduceTerm(m, removal_pairs)
    for u_deg in u_degree:
        for u_char_tuple in product('abcghy', repeat=degree):
            u = ''.join(u_char_tuple)
            if not u in test_u:
                test_u[u] = 'f'
            if test_u[u] == 'r':
...
for monomial_char_tuple in product('abcghy', repeat=degree):
    ...
    for u_degree in [2,4,6]:
        for u_char_tuple in product('abcghy', repeat=u_degree):
            ...
            ###DEBUG: ENABLE FOR SHORTER u SEQUENCES
